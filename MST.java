...
public class MST {
    ...
    public static int getThreadCount(){return numThreads;}
...
//The Helpers are threads that assist the worker or main thread in finding a triangulation and MST
//(They function very similar to worker and call Coordinator)
class Helper extends Thread{
    ...
    // Constructor
    //
    public Helper(int start, int end,MSTworld w , Coordinator c) {
        this.start=start;
        this.end=end;
        this.w=w;
        this.c=c;
...
//The TraingleHelpers are threads that assist the worker or main thread in finding a triangulation and MST
//(They function very similar to worker and call Coordinator)
class TriangleHelper extends Thread{
    ...
    // Constructor
    //
    public TriangleHelper(int l, int r, int low0, int high0, int low1, int high1, int parity, MSTworld w , Coordinator c, Semaphore s) {
        this.w=w;
        this.c=c;
        this.l=l;
        this.r=r;
    ...
    public void run() {
        try {
            c.register();
            //Makes sure lock is acquired so we don't have conflicting threads trying to access the same things
            try{
                if (s.tryAcquire(1)){
                    lock.lock();
                    try{
                        w.triangulate(l, r, low0, high0, low1, high1, parity);
                    }finally{
                        lock.unlock();
                        s.release();
                    }
...
class MSTworld {
    ...
    int threads=MST.getThreadCount();
    int tused=0;
    TriangleHelper [] array= new TriangleHelper [threads];
    ...
    //Hashmap which is used by Helper thread to mark cycles in the graph to prevent main thread from unnecessairly checking them
    private HashMap<edge,Boolean> cycle =new HashMap<edge,Boolean>();
    ...
    //
    public void triangulate(int l, int r, int low0, int high0,
        ...
        triangulate(l, i, low1, high1, low0, mid, 1-parity);
            ...
            // divide and conquer
            //Checks to make sure that we actually can make threads
            //Makes sure active threads isn't more than amount allowed by input value
            if (Thread.activeCount() < threads){
                //Splits the graph into subsections handing some of the work off to a helper thread
                TriangleHelper t= new TriangleHelper(j, r, low1, high1, mid, high0, 1-parity, this , coord, sem);
                t.start();
                triangulate(l, i, low1, high1, low0, mid, 1-parity);
                try{
                    t.join();
    ...
    //Funciton to allow Threads to speed up Kruskal's algorithm
    //Uses split edge set to allow helper thread to check for cycles in the graph
    //Speeds up process by throwing out threads that are cycles based on information provided by main graph
    //Uses hashmap with booleans to point out cycles
    public void KruskalHelp(int start, int end)
        throws Coordinator.KilledException{
            int ss=0;
            int ie=0;
            edge se=null;
            ...
            SortedSet<edge> sub= edges.subSet(se,ee);
            for(edge e: sub){
    ...
    public void KruskalSolve()
        ...
        int numThreads=MST.getThreadCount();
        int m=1;
        int start=0;
        double answer=Math.floor(length/numThreads);
        int divide= (int)answer;
        for (int i=0;i<numThreads-1;i++){   
            if (length-(m+1)*divide<0){
                Helper t = new Helper (start,length, this, coord);
                t.start();
                break;
            }
            int end=m*divide;    
            Helper t= new Helper(start,end, this, coord);
            start=end+1;
            m++;
            t.start();
        }
